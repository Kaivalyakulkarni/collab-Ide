...
export default function EditorPage({
  ...
  const handleContentChange = (content: string) => {
    ...
    saveTimer.current = setTimeout(() => {
      ...
      sendInitRef.current?.(
        content,
        getLanguageFromFileName(selectedFileRef.current!.name),
  ...
  const sendInitRef = useRef<
    ((content: string, language: string, fileName: string) => void) | null
  >(null);
  const [activePanel, setActivePanel] = useState<"files" | "git">("files");

  const [aiStrength, setAiStrength] = useState<"off" | "normal" | "aggressive">(
    "normal",
  );
  ...
  if (isMobile) {
    return (
      ...
      >
        ...
        <div
          style={{ fontSize: "11px", color: "#7F8C8D", letterSpacing: "0.1em" }}
        >
          // editor.desktop_only()
        </div>
        <div
        ...
        >
          The code editor needs a bigger screen to work properly.
        </div>
        <div
          style={{
            fontSize: "12px",
        ...
        >
          Open this project on a laptop or desktop to write code, use the
          terminal, and manage git.
        </div>
        <button
    ...
    >
      ...
      <div style={{ display: "flex", flex: 1, overflow: "hidden" }}>
        ...
        >
          <div
            ...
            style={{
              ...
              justifyContent: "center",
              cursor: "pointer",
              borderLeft: `2px solid ${activePanel === "files" ? "#BDC3C7" : "transparent"}`,
              color: activePanel === "files" ? "#BDC3C7" : "#555",
          ...
          <div
            ...
            style={{
              ...
              justifyContent: "center",
              cursor: "pointer",
              borderLeft: `2px solid ${activePanel === "git" ? "#BDC3C7" : "transparent"}`,
              color: activePanel === "git" ? "#BDC3C7" : "#555",
              transition: "all 0.15s",
            }}
          >
            <svg
        ...
        >
          ...
          >
            {[...new Map(users.map((u) => [u.name, u])).values()].map(
              (user) => (
                <div
                  key={user.name}
                  title={user.name}
      ...
      >
        ...
        <span style={{ marginLeft: "auto", display: "flex", gap: "16px" }}>
          <span>
            {activeTab
              ? getLanguageFromFileName(activeTab.name).toUpperCase()
              : "—"}
          </span>
          <span
            onClick={() => {
